...
";
	...
	static function html_table_summary($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($stock=$result->fetch_object()){
			...
			if($action){
				...
				$action_buttons.= Event::button(["id"=>$stock->sid, "name"=>"Details", "value"=>"Details", "class"=>"info", "route"=>"stock/show/$stock->id"]);
	...
	static function stock_details($product_id,$page = 1,$perpage = 10,$criteria="",$action=true){
		global $db,$tx;
		$count_result =$db->query("select count(*) total from {$tx}stocks $criteria ");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;
		$result=$db->query("select s.id,p.name,s.qty,t.name ttype,s.remark,w.name warehouse from {$tx}stocks s,{$tx}products p,{$tx}transaction_types t,{$tx}warehouses w where s.product_id=p.id and p.id=$product_id and t.id=s.transaction_type_id and w.id=s.warehouse_id  $criteria limit $top,$perpage");
		$html="<table class='table'>";
			
		if($action){
			$html.="<tr><th>Id</th><th>Product</th><th>Qty</th><th>Transaction Type</th><th>Remark</th><th>Warehouse</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Product</th><th>Qty</th><th>Transaction Type</th><th>Remark</th><th>Warehouse</th></tr>";
		}
		$total=0;
		while($stock=$result->fetch_object()){
			$action_buttons = "";
			$total+=$stock->qty;
			$html.="<tr><td>$stock->id</td><td>$stock->name</td><td>$stock->qty</td><td>$stock->ttype</td><td>$stock->remark</td><td>$stock->warehouse</td></tr>";
		}
		$html.="<tr><td colspan='2'><b>Total</b></td><td><b>$total</b></td><td colspan='2'></td></tr>";
		$html.="</table>";
		$html.= pagination($page,$total_pages);
